...
({params, imports}) => {
  ...
  const lmp = programEncounter.programEnrolment.getObservationValue('7bfef905-d0c1-45de-8e93-2e58657845fd');
  ...
  if (observationDate && lmp && moment(observationDate).isSameOrBefore(moment(lmp), 'day')) {
    validationErrors.push('Observation date should be after LMP');
  }
...
({ params, imports }) => {
  ...
  let dateOfDeliveryObservation = programEncounter.findObservation('5f9a5098-d052-4843-8e13-0c1f455490fc');
  let dateOfDeliveryValue = null;

  if (!_.isEmpty(dateOfDeliveryObservation)) {
    const rawValue = dateOfDeliveryObservation.getValue(); 
    if (rawValue) {
      dateOfDeliveryValue = moment(rawValue).toDate(); 
    }
  }

  let condition11 = false;
  if (dateOfDeliveryValue) {
    condition11 = new imports.rulesConfig.RuleCondition({ programEncounter, formElement })
      .when.valueInEncounter("38ff4d5f-65be-45b0-9304-8c3fad41afc7")
      .lessThan(dateOfDeliveryValue, 'ms')
      .matches();
  }
  ...
  const condition12 = new imports.rulesConfig.RuleCondition({ programEncounter, formElement })
    .when.valueInEncounter("e465d95c-b64d-4c46-aa28-73e8cb98f8a3")
    .containsAnswerConceptName("f7a3a360-58d3-4987-99ff-e7fb97f911a0")
    .matches();
...
({params, imports}) => {
  ...
  if (!_.isEmpty(dateOfDeliveryObservation)) {
    const rawValue = dateOfDeliveryObservation.getValue(); // raw ISO or date object
    if (rawValue) {
      dateOfDeliveryValue = moment(rawValue).toDate(); // convert safely
    }
  }
  ...
  if (dateOfDeliveryValue) {
    condition11 = new imports.rulesConfig.RuleCondition({programEncounter, formElement})
      .when.valueInEncounter("683bed56-8e81-4f5a-b3e4-5c88076616c7")
      .lessThan(dateOfDeliveryValue, 'ms')
      .matches();
  }
  ...
  const condition12 = new imports.rulesConfig.RuleCondition({programEncounter, formElement})
    .when.valueInEncounter("f2bddcfb-4721-4a39-a426-ff68ec050260")
    .containsAnswerConceptName("f7a3a360-58d3-4987-99ff-e7fb97f911a0")
    .matches();
