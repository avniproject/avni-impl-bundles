...
({ params, imports }) => {
  ...
  console.log('encounter in follow up--->',encounter);
  
  const isNewEncounter = !encounter.earliestVisitDateTime;
  
  console.log('isNewEncounter--->',isNewEncounter)

  if (!isNewEncounter) {
    //return scheduleBuilder.getAll(); // skip scheduling
  }

  const dob = encounter.individual.dateOfBirth;
  const today = moment();
  const ageInYears = moment(today).diff(moment(dob), 'years', true);  // accurate decimal age
